# source: https://github.com/today-is-a-good-day/Emoticons/blob/master/scrapeEmoticons.R
# libraries to load
library(rvest)
library(magrittr)
library(dplyr)
...
# get uncategorised emoticons
uncategorised <- url %>%
  read_html() %>%
  html_nodes(xpath='/html/body/div[2]/div/div/table[5]') %>%
  html_table()
uncategorised <- data.frame(uncategorised[[1]]$Native, uncategorised[[1]]$Bytes, 
                            uncategorised[[1]]$Description, stringsAsFactors = FALSE)
names(uncategorised) <- c("Native", "Bytes", "Description")
...
alltogether <- bind_rows(list(emoticons, addemoticons, dingbats, transport, 
                              addtransport, enclosed, uncategorised, addothers))